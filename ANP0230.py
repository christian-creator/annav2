...
from excel_plot import write_and_plot_excel_ANP0230,write_and_plot_excel_ANP0020


def get_ANP0230_data(slut_dato):
    ...
    for file in os.listdir("data/ANP0230/raw"):
        ...
        data = np.delete(data, [2, 4, 5,6], axis=1)
        data_fixed = dict()
        ...
        for row in data:
            ...
            if len(date) < 3:
                print(room,"Date is missing in ANP0230")
                continue
        ...
        if room in room_classes.keys():
            data_fixed['alert'] = [
                int(room_classes[room]["ANP0230_alert"]) for i in range(len(data_fixed))]
            # data_fixed.to_excel("data/ANP0230/filtered/{}.xlsx".format(room))
            write_and_plot_excel_ANP0230(data_fixed, room, slut_dato)
        else:
            pass
